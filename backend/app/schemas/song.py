...
class SongDTO(BaseModel):
    ...
    duration_str: str = "0:00"  # Formatted duration M:SS for frontend player sync
    ...
    def populate_derived_fields(cls, data: Any) -> Any:
        ...
        if not isinstance(data, dict):
            ...
            extracted["artist"] = getattr(data, "artist", None)
            extracted["album"] = getattr(data, "album", None)
            data = extracted

        if isinstance(data, dict):
            # Replace None with schema defaults for non-optional fields (handles unflushed ORM instances)
            ...
            if not data.get("artist_name") or data.get("artist_name") == "Unknown Artist":
                artist = data.get("artist")
                if isinstance(artist, dict) and artist.get("name"):
                    data["artist_name"] = artist["name"]
                elif hasattr(artist, "name") and getattr(artist, "name"):
                    data["artist_name"] = getattr(artist, "name")
                else:
                    data["artist_name"] = data.get("artist_name") or "Unknown Artist"

            dur = data.get("duration")
            dur_val = 0 if dur is None else dur
            data["duration_str"] = f"{dur_val // 60}:{dur_val % 60:02d}"

            # Tag list derivation if tags string is present and tag_list is empty/None
            if not data.get("tag_list") and data.get("tags"):
